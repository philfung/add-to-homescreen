...
class AddToHomeScreen {
    ...
    constructor({ appName, appIconUrl, assetUrl, maxModalDisplayCount, }) {
        this._genDesktopChrome = (container) => {
            var blurb = this.isDesktopMac()
                ? this._genBlurbDesktopMac()
                : this._genBlurbDesktopWindows();
      ...
      <button class="adhs-button adhs-button-install">
        ` +
            ...
            installButton.addEventListener("click", () => {
                ...
                this.closeModal();
                this._desktopInstallPromptEvent.userChoice.then((choiceResult) => {
        ...
        this._desktopInstallPromptEvent = null;
        this._desktopInstallPromptWasShown = false;
        this._desktopInstallPromptEventListener = (e) => {
        ...
        if (this.shouldShowDesktopInstallPromptBasedOnDevice()) {
            this._registerDesktopInstallPromptEvent =
                this._registerDesktopInstallPromptEvent.bind(this);
            this._registerDesktopInstallPromptEvent();
        }
    }
    isStandAlone() {
    ...
    show(locale = "en") {
        ...
        if (this.isStandAlone()) {
            ...
            ret = new DeviceInfo((isStandAlone = true), (canBeStandAlone = true), (device = device));
        }
        else if (this._hasReachedMaxModalDisplayCount()) {
            ret = new DeviceInfo((isStandAlone = false), (canBeStandAlone = false), (device = device));
        }
            ...
            if (this.isDeviceIOS()) {
                ...
                if (this.isBrowserIOSSafari()) {
                    ret = new DeviceInfo((isStandAlone = false), (canBeStandAlone = true), (device = device));
  ...
  AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/114.1 Mobile/15E148 Safari/605.1.15 */
    ...
    isBrowserAndroidFacebook() {
        return (this.isDeviceAndroid() &&
            window.navigator.userAgent.match(/FBAN|FBAV/));
    }
  ...
  (KHTML, like Gecko) SamsungBrowser/21.0 Chrome/110.0.5481.154 Mobile Safari/537.36 */
    ...
    _assertArg(variableName, booleanExp) {
        if (!booleanExp) {
            throw new Error("AddToHomeScreen: variable '" +
                variableName +
    ...
    _createContainer(include_modal = false) {
        ...
        if (include_modal) {
            var containerInnerHTML = this._genLogo() + this._genModalStart() + this._genModalEnd();
            container.innerHTML = containerInnerHTML;
        }
        return container;
    }
    ...
    _genListButtonWithImage(imageUrl, text = "", image_side = "none") {
        if (!text) {
            return (`
    ...
    _genIOSSafari(container) {
        var containerInnerHTML = this._genLogo() +
            ...
            this._genListItem(`2`, i18n.__("Select %s from the menu that pops up.", this._genListButtonWithImage(this._genAssetUrl("ios-safari-add-to-home-screen-button-2.svg"), i18n.__("Add to Home Screen"), "right")) +
    ...
    _genDesktopSafari(container) {
      ...
      <img src="` +
            this._genAssetUrl("desktop-safari-bouncing-arrow.svg") +
            `" alt="arrow" />
    </div>`;
        container.innerHTML = containerInnerHTML;
        container.classList.add("adhs-desktop");
    ...
    _isEnabledModalDisplayCount() {
        return (typeof this.maxModalDisplayCount === "number" &&
    ...
    _getModalDisplayCount() {
        var countStr = window.localStorage.getItem(this._getModalDisplayCountKey());
        var count;
        if (countStr === null) {
            count = 0;
            window.localStorage.setItem(this._getModalDisplayCountKey(), count.toString());
